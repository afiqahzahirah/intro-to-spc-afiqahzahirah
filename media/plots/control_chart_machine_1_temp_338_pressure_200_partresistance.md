...
chart_data <- filtered_data$PartResistance
...
p <- ggplot(xbar_df, aes(x = x, y = y)) +
  ...
  labs(
    title = "X-bar.one Chart for PartResistance (Machine 1, 338K, 200kPa)",
    x = "Observation",
    y = "PartResistance"
  ) +
  theme_minimal() +
...
htmlwidgets::saveWidget(p_plotly, "media/plots/control_chart_machine_1_temp_338_pressure_200_partresistance.html", selfcontained = TRUE)
